...
class TestMCPServerStartup:
    ...
    def test_memory_server_tool_list(self):
        ...
        assert len(tool_names) == 10, f"Expected 10 memory tools, got {len(tool_names)}: {tool_names}"
    ...
    def test_tools_server_tool_list(self):
        ...
        for name in [
            ...
            assert name in tool_names, f"Missing tool: {name}"
        assert len(tool_names) == 30, f"Expected 30 domain tools, got {len(tool_names)}: {tool_names}"
