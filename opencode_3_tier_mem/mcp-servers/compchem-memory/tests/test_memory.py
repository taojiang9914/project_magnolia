...
class TestProjectManager:
    ...
    def test_entry_with_rich_metadata(self, project_dir):
        """Verify that entries can carry metadata for future Skill use."""
        mgr = ProjectManager(project_dir)
        mgr.create_entry(
            str(project_dir),
            "Rich Entry",
            "Content with metadata",
            tags=["docking", "haddock3"],
            source="auto",
            entry_type="success_pattern",
            tools=["haddock3", "preprocess_pdb"],
            confidence=0.9,
        )
        entries = mgr.list_entries(str(project_dir))
        assert len(entries) == 1
        e = entries[0]
        assert e["type"] == "success_pattern"
        assert e["confidence"] == 0.9
        assert "haddock3" in e["tools"]
        assert "docking" in e["tags"]
